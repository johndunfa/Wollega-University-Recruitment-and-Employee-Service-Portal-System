...
interface LeaveRequestData {
  ...
  // New field to track manager approval
...
const ManagerDashboard = () => {
  ...
  const [pendingRequests, setPendingRequests] = useState<LeaveRequestData[]>([])
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (role !== "manager" && role !== "admin") {
          setStatusMessage("Access denied. Manager privileges required.")
          setMessageType("error")
          return
        }
  ...
  const fetchAllRequests = async () => {
    try {
      ...
      // Show requests that are pending manager approval
      setPendingRequests(requests.filter((req: LeaveRequestData) => 
        req.managerApproval === "pending" || req.status === "pending"
      ))
    } catch (error) {
  ...
  const handleApproveReject = async (requestId: string, action: "approved" | "rejected", comments = "") => {
    ...
    try {
      const res = await fetch("/api/manager/approve-leave", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          requestId,
      ...
      if (res.ok) {
        if (action === "approved") {
          setStatusMessage("Leave request approved! Sent to HR for final approval.")
        } else {
          setStatusMessage("Leave request rejected successfully!")
        }
  ...
  const getStatusIcon = (status: string, managerApproval?: string, hrApproval?: string) => {
    // For backward compatibility, check status first
  ...
  const getStatusText = (status: string, managerApproval?: string, hrApproval?: string) => {
    // For backward compatibility, check status first
    if (status.toLowerCase() === "approved") return "Approved"
    if (status.toLowerCase() === "rejected") return "Rejected"
    
    // New workflow statuses
    if (managerApproval === "rejected") return "Rejected by Manager"
    if (managerApproval === "approved" && hrApproval === "approved") return "Approved by HR"
    if (managerApproval === "approved") return "Pending HR Approval"
    
    return "Pending Manager Approval"
  }

  const getStatusColor = (status: string, managerApproval?: string, hrApproval?: string) => {
    // For backward compatibility, check status first
    if (status.toLowerCase() === "approved") {
  ...
  const filteredRequests = allRequests.filter((req) => {
    const matchesStatus = filterStatus === "all" || 
      (filterStatus === "pending" && (req.managerApproval === "pending" || req.status === "pending")) ||
      (filterStatus === "approved" && (req.status.toLowerCase() === "approved" || 
        (req.managerApproval === "approved" && req.hrApproval === "approved"))) ||
      (filterStatus === "rejected" && (req.status.toLowerCase() === "rejected" || 
      ...
      <div className="bg-gradient-to-r from-[#087684] to-[#066466] rounded-xl p-8 text-white">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[#B8E6EE]">Review and manage team leave requests</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100">
        <div className="flex items-center gap-4">
          ...
          <div className="text-right">
            ...
            <p className="text-2xl font-bold text-[#087684]">{pendingRequests.length}</p>
          </div>
        </div>
      </div>
          ...
          {loading ? (
            ...
            <div className="space-y-4">
              {filteredRequests.map((req, idx) => {
                const canReview = req.managerApproval === "pending" || req.status === "pending";
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-3">
                          ...
                          <div
                            className={`px-3 py-1 rounded-full text-sm font-medium ${getLeaveTypeColor(req.leaveType)}`}
                          >
                            {req.leaveType} Leave
                          </div>
                          <div
                            className={`flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium border ${getStatusColor(req.status, req.managerApproval, req.hrApproval)}`}
                          >
                            {getStatusIcon(req.status, req.managerApproval, req.hrApproval)}
                            {getStatusText(req.status, req.managerApproval, req.hrApproval)}
                          </div>
                        </div>
